...
public interface ScheduleRepository extends R2dbcRepository<Schedule, Long>, ScheduleRepositoryInternal {
    ...
    @Query("SELECT * FROM schedule entity WHERE entity.sch_fit_id = :id")
    Flux<Schedule> findBySchFit(Long id);

    @Query("SELECT * FROM schedule entity WHERE entity.sch_fit_id IS NULL")
    Flux<Schedule> findAllWhereSchFitIsNull();
